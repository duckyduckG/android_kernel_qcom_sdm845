...
static int fts_gpio_setup(int gpio, bool config, int dir, int state);//SW5_DH_A1P_ST_TOUCH+
...
static ssize_t fts_strength_frame_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    int res= ERROR_OP_NOT_ALLOW, count = -1, j, size =6*2;
    u16 type = 0;
...
//SW5_DH_A1P_ST_TOUCH+[
#if 1
    logError(0, "%s %s: Skipped in A1P project\n", tag, __func__);
    return 0;
...
static int fts_fb_state_chg_callback(struct notifier_block *nb, unsigned long val, void *data)
{
    ...
    if (evdata && evdata->data && val == FB_EVENT_BLANK && info)
    {
        ...
        case FB_BLANK_UNBLANK:
            ...
            break;
          //SW5_DH_A1P_ST_TOUCH-[
          //SW5-DH-for_AmbientDisplay-00+[
          #if 0
       case FB_BLANK_NORMAL:
            if(!info->sensor_sleep)
                break;
            logError(0, "%s %s: FB_BLANK_NORMAL\n", tag, __func__);
            queue_work(info->event_wq, &info->resume_work);
            break;
            //SW5-DH-for_AmbientDisplay-00+]
        #endif
                          ...
                          bool enable)
{
    ...
    //SW5-DH_A1P_ST_TOUCH+[
     if (bdata->tp_1p8_en >= 0)
    {
        retval = fts_gpio_setup(bdata->tp_1p8_en, true, 1, 1);
        if (retval < 0)
        {
            logError(1, "%s %s: Failed to configure tp_1p8_en GPIO\n", tag, __func__);
            goto exit;
        }
    }
    //SW5-DH_A1P_ST_TOUCH+]
...
disable_bus_reg:
    ...
    //SW5-DH_A1P_ST_TOUCH+[
    retval = fts_gpio_setup(bdata->tp_1p8_en, true, 1, 0);
        if (retval < 0)
        {
            logError(1, "%s %s: Failed to configure tp_1p8_en GPIO\n", tag, __func__);
            goto exit;
...
static int parse_dt(struct device *dev, struct fts_i2c_platform_data *bdata)
{
    ...
    //SW5-DH_A1P_ST_TOUCH+[
    if (of_property_read_bool(np, "TP_LDO_1P8_EN"))
    {
        bdata->tp_1p8_en = of_get_named_gpio_flags(np,
                            "TP_LDO_1P8_EN", 0, NULL);
        logError(0, "%s tp_1p8_en =%d\n", tag, bdata->tp_1p8_en);
    }
    else
    {
        bdata->tp_1p8_en = GPIO_NOT_DEFINED;
    }
    //SW5-DH_A1P_ST_TOUCH+]
                     ...
                     const struct i2c_device_id *idp)
{
    ...
    {
        printk("%s %s: Another driver probe success. skip FTS touch driver!\n", tag, __func__);
        return 0;
    }
...
//    wake_lock_init(&info->wakelock, WAKE_LOCK_SUSPEND, "fts_tp");
    ...
    //input_set_capability(info->input_dev, EV_KEY, KEY_AMBIENT_EVENT);// Ambient Display
