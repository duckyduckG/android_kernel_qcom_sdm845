...
//FIH: Reading cal data from mfd during init
...
#define TAS2557_CAL_DEFAULT	"/vendor/firmware/AOP_TAS2557_CAL.bin"
...
static int tas2557_load_calibration_FIH(struct tas2557_priv *pTAS2557);
...
{
	printk("BBox::EHCS;52001:i:%s:FW Version=0x%04X,DDC Name=%s\n",__func__,pFirmware->mnFWVersion,pFirmware->mpDDCName);
...
static int tas2557_load_calibration(struct tas2557_priv *pTAS2557,	char *pFileName)
{
	...
	printk("BBox::EHCS;52001:i:%s:Loading calibrarion data from %s\n",__func__,pFileName);
...
static int tas2557_load_calibration_FIH(struct tas2557_priv *pTAS2557)
{
	...
	dev_info(pTAS2557->dev, "%s: enter\n", __func__);
	printk("BBox::EHCS;52001:i:%s:Loading calibrarion data from MFD\n",__func__);
	...
	dev_info(pTAS2557->dev, "TBC: sizeof(reginfo)=%ld, start memcpy from %p\n", sizeof(reginfo), (void*)buf);
	...
	if (reginfo.len > 0) {
		nSize = reginfo.len;
		dev_info(pTAS2557->dev, "TBC: nSize=%d, start memcpy from %p\n", nSize, (void*)(buf+sizeof(reginfo)));
	...
	dev_info(pTAS2557->dev, "TBC: sizeof(reginfo_tail)=%ld, start memcpy from %p\n", sizeof(reginfo_tail), (void*)(buf+sizeof(reginfo)+nSize));
	memcpy(&reginfo_tail,  (void *)(buf+sizeof(reginfo)+nSize), sizeof(reginfo_tail));
	dev_info(pTAS2557->dev, "Magic=0x%x,Reserved=0x%x\r\n", reginfo_tail.Magic, reginfo_tail.Reserved);

	iounmap(buf);

	if (reginfo.Magic != SNS_REG_MAGIC_V02) {
		if(reginfo.Magic == SNS_REG_MAGIC_V01) {
			dev_err(pTAS2557->dev, "Verify Calibrarion data warning: old Magic:0x%x \r\n", reginfo.Magic);
			//printk("BBox::TRG;%s::%d::%s\n", "RUN", 0, "rt_smartamp_calibration");
			printk("BBox::EHCS;52001:i:%s:Skip loading old calibrarion data: Magic:0x%x\n",__func__,reginfo.Magic);
		} else {
			dev_err(pTAS2557->dev, "Verify Calibrarion data fail: wrong Magic:0x%x \r\n", reginfo.Magic);
			printk("BBox::EHCS;52001:i:%s:Skip loading wrong Calibrarion data: Magic:0x%x\n",__func__,reginfo.Magic);
		}
		return -EINVAL;
	}
	...
	if (reginfo_tail.Magic!=SNS_REG_MAGIC_TAIL) {
		...
		printk("BBox::EHCS;52001:i:%s:Verify Calibrarion data fail: wrong tail Magic:0x%x\n",__func__,reginfo.Magic);
		return -EINVAL;
    }
...
int tas2557_set_calibration(struct tas2557_priv *pTAS2557, int nCalibration)
{
	...
	if (nCalibration == 0x00FF) {
		/* FIH modified start */
		/*
			Loading from loading data from MFD (generated from Factorytest and stored in MFD)
			if still failed, loading data from /vendor/firmware/AOP_TAS2557_CAL.bin
		*/
		nResult = tas2557_load_calibration_FIH(pTAS2557);
		if (nResult < 0) {
			nResult = tas2557_load_calibration(pTAS2557, TAS2557_CAL_DEFAULT);
		}
		if (nResult < 0) {
			dev_info(pTAS2557->dev, "load new calibration file fail %d\n", nResult);
			printk("BBox::EHCS;52001:i:%s:Loading calibrarion data fail %d\n", __func__, nResult);
			goto end;
		}
		/* FIH modified end */
		nCalibration = 0;
...
int tas2557_parse_dt(struct device *dev, struct tas2557_priv *pTAS2557)
{
	...
	if (!rc) {
		...
		if (strcmp(device_model, "AOP") == 0) {
			tas2557_fw_bin = "AOP_TAS2557.bin";
		} else {
